...
def group_items(scene, context):
    ...
    if not tina_to_active and len(selection) == 2:
        for ob in selection:
            if ob != target:
                target = ob
...
class FMDL_TransferNormals(bpy.types.Operator):
    ...
    def draw(self, context):
        ...
        global tina_to_active
        if tina_to_active or len(selection) == 2:                
...
# Transfer normals from or to selection        
        for ob in selection:
            if active != ob and ob.type == "MESH":
                ...
                if self.to_active or len(selection) == 2:
    ...
    def execute(self, context):
        ...
        for axis in axes:
            ...
            array.constant_offset_displace = [
                self.offset_x if axis == 'X' else 0,
                self.offset_y if axis == 'Y' else 0,
                self.offset_z if axis == 'Z' else 0
            ]
