...
def _pes_find_image(*keywords):
	for img in bpy.data.images:
		nm = (img.name or '').lower()
		if any(k in nm for k in keywords):
			return img
	return None

def _pes_ensure_node_material(mat, image):
	# base_file.blend ships Blender-Internal (2.79) materials for eyes/mouth that
	# do not render in Blender 2.8+/4.x/5.x. Convert them to a Principled node tree.
	if mat is None:
		return
	try:
		mat.use_nodes = True
	...
	if not any(l.from_node == principled and l.to_node == output for l in links):
		try:
			links.new(principled.outputs['BSDF'], output.inputs['Surface'])
		except Exception:
			pass
	if image is not None and not any(n.type == 'TEX_IMAGE' and n.image == image for n in nodes):
		texNode = nodes.new('ShaderNodeTexImage')
		texNode.image = image
		texNode.location = (-400, 0)
		try:
			links.new(texNode.outputs['Color'], principled.inputs['Base Color'])
		except Exception:
			pass
		try:
			if 'Alpha' in principled.inputs:
				links.new(texNode.outputs['Alpha'], principled.inputs['Alpha'])
				mat.blend_method = 'HASHED'
		except Exception:
			pass

def fix_base_scene_materials():
	mapping = {'eyeL': ('eye',), 'eyeR': ('eye',), 'mouth': ('mouth', 'oral')}
	for obName, keys in mapping.items():
		ob = bpy.data.objects.get(obName)
		if ob is None:
			continue
		img = _pes_find_image(*keys)
		for slot in ob.material_slots:
			_pes_ensure_node_material(slot.material, img)
...
def scene_objects():
	...
	try:
		fix_base_scene_materials()
	except Exception:
		pass
	# Match the original 2.79/2.92 "Blender Render" look. Blender 4.x defaults to the
	# AgX view transform, which heavily desaturates skin tones and makes the face look
	# grey/ashy. Force Standard so the skin reads as the warm colour in the texture.
	try:
		...
		pass
	# Switch 3D viewports to Material Preview so imported textures / skin colour are
	# actually visible. In Solid shading mode Blender shows a flat grey clay and
	# hides all textures, which looks identical to a 'grey skin' bug.
	try:
...
def texconv(inPath, outPath, arguments, cm):
	# Guard: the .dds may be missing if a .ftex failed to convert. Skip gracefully
	# instead of crashing the whole extraction with FileNotFoundError.
	if not os.path.isfile(inPath):
		print('texconv: skip, file not found: {0}'.format(inPath))
		return 0
	try:
		...
		print('texconv: skip, cannot read {0}: {1}'.format(inPath, e))
		return 0
	if cm:
		if TxFormat == "DX10":
...
def texture_load(dirPath):
	# Initialise so we never raise UnboundLocalError when the directory is empty
	# or does not exist (e.g. extraction produced no #windx11 folder).
	root, directories, filenames = dirPath, [], []
	if not os.path.isdir(dirPath):
		print('texture_load: directory not found, skipping: {0}'.format(dirPath))
		return root, directories, filenames
	for root, directories, filenames in os.walk(dirPath):
		for fileName in filenames:
			filename, extension = os.path.splitext(fileName)
			if extension.lower() == '.ftex':
				...
				try:
					...
					print('texture_load: ftexToDds failed for {0} ({1}); trying FtexTools'.format(ftexPath, e))
					try:
						convert_ftex(ftexPath)
					except Exception as e2:
						print('texture_load: convert_ftex also failed for {0}: {1}'.format(ftexPath, e2))
				# Only run texconv if the .dds was actually produced.
				if os.path.isfile(ddsPath):
					try:
						texconv(ddsPath, dirPath, " -r -y -f DXT5 -ft dds -o ", True)
					except Exception as e:
						print('texture_load: texconv failed for {0}: {1}'.format(ddsPath, e))
				else:
					print('texture_load: no .dds produced for {0}, skipping texconv'.format(ftexPath))
...
def NewID():
	...
	for ob in bpy.data.objects:
		for mat_slot in ob.material_slots:
			for mtex_slot in mat_slot.material.fmdl_texture_slots:
				if mtex_slot is not None:
					if mtex_slot.texture is not None and mtex_slot.texture.fmdl_texture_role and '_Tex_' in mtex_slot.texture.fmdl_texture_role:	
						texname = mtex_slot.texture.fmdl_texture_directory
						if 'real' in texname:
							IDFinds = re.findall(r'\w+', old_path)
							texname = texname.replace(texname, "/Assets/pes16/model/character/face/real/%s/sourceimages/" % NEW_ID)
							mtex_slot.texture.fmdl_texture_directory = texname
	for i in range(100):
		if len(IDFinds) == i+1:
			OLD_ID2 = IDFinds[i]
	if is_number(OLD_ID):
...
class FMDL_Scene_Extract_Fpk(bpy.types.Operator, bpy_extras.io_utils.ImportHelper):
	...
	if not config.has_section('bl_sett'):
		config.add_section('bl_sett')
	for _bk, _bv in (('face_high', 'True'), ('hair_high', 'True'), ('oral', 'True'), ('pes_diff', 'True')):
		if not config.has_option('bl_sett', _bk):
			config.set('bl_sett', _bk, _bv)
	...
	def execute(self, context):
		...
		os.makedirs(os.path.dirname(ini_sett), exist_ok = True)
...
class FMDL_Scene_Open_Image(bpy.types.Operator, bpy_extras.io_utils.ImportHelper):
	...
	def execute(self, context):
		...
		_idx_open = material.fmdl_active_texture_index
		if not (0 <= _idx_open < len(material.fmdl_texture_slots)) or material.fmdl_texture_slots[_idx_open].texture is None:
			self.report({"WARNING"}, "Select a texture slot in the FMDL Material panel first!!")
			return {'CANCELLED'}
		_activeTex = material.fmdl_texture_slots[_idx_open].texture
		texName = _activeTex.name
		texture_role = _activeTex.fmdl_texture_role
		idx = _idx_open
		...
		_activeTex.name = "[%s] %s" % (texture_role, fileName)
		texname = _activeTex.name
...
class FMDL_Mesh_BoneGroup_Specials(bpy.types.Menu):
	...
	def draw(self, context):
		...
		self.layout.operator(FMDL_Mesh_BoneGroup_CopyFromSelected.bl_idname, icon='LINKED')
...
# Module-level guard to break the mutual recursion between the IntProperty
# update callbacks (update_*_enum) and the EnumProperty update callbacks
# (update_*_list). Without this, setting one property triggers the other,
# which sets the first again -> infinite recursion -> native stack-overflow
# crash in Blender 4.x.
_pes_enum_sync_guard = False

def _validEnumId(items, value):
	# Returns str(value) if it is a valid identifier in the given enum items
	# list, otherwise 'Unknown'. Assigning an identifier that is NOT present in
	# an EnumProperty's items hard-crashes Blender 4.x, so we must validate
	# BEFORE assigning.
	target = str(value)
	for item in items:
		if item[0] == target:
...
def update_alpha_list(self, context):
	global _pes_enum_sync_guard
	if _pes_enum_sync_guard:
		return
	_pes_enum_sync_guard = True
	try:
		try:
...
def update_alpha_enum(self, context):
	...
	_pes_enum_sync_guard = True
	try:
		self.fmdl_alpha_enum_select = _validEnumId(PesFoxShader.AlphaEnum, self.fmdl_alpha_enum)
	except:
		pass
	finally:
		_pes_enum_sync_guard = False
...
def update_shadow_enum(self, context):
	...
	_pes_enum_sync_guard = True
	try:
		self.fmdl_shadow_enum_select = _validEnumId(PesFoxShader.ShadowEnum, self.fmdl_shadow_enum)
	except:
		pass
	finally:
		_pes_enum_sync_guard = False
...
class FMDL_TextureSlot(bpy.types.PropertyGroup):
...
class FMDL_Externally_Edit(bpy.types.Operator):
	...
	def execute(self, context):
		_m = bpy.context.active_object.active_material
		_i = _m.fmdl_active_texture_index
		if 0 <= _i < len(_m.fmdl_texture_slots) and _m.fmdl_texture_slots[_i].texture is not None:
			texname = _m.fmdl_texture_slots[_i].texture.name
		else:
			texname = str()
		fileName = str()
		imagePath = str()
...
def register():
	...
	bpy.utils.register_class(FMDL_TextureSlot)
	bpy.types.Material.fmdl_texture_slots = bpy.props.CollectionProperty(type=FMDL_TextureSlot)
	bpy.types.Material.fmdl_active_texture_index = bpy.props.IntProperty(name="FMDL active texture", default=-1)
...
def unregister():
	...
	bpy.utils.unregister_class(FMDL_TextureSlot)
